...
namespace {
// Physical pin 22 = GPIO25 = TRIG ("TX": this MCU pulses it to fire a ping).
// Physical pin 24 = GPIO8  = ECHO ("RX": this MCU reads the return pulse).
// GPIO8 is SPI0 CE0 under its ALT function, but is a plain free GPIO as
// long as SPI0 isn't enabled/in use elsewhere on this board.
constexpr unsigned TRIG_PIN = 25;
constexpr unsigned ECHO_PIN = 8;

// Identifies this sensor to the supervisor, since ultrasonic_task-1 sends
// the same message types on the same channel.
constexpr uint8_t SENSOR_ID = ULTRASONIC_ID_2;
